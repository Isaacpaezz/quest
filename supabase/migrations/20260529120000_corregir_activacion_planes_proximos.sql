-- BUG FIX: Los planes próximos no se activan si el plan anterior ya fue completado
-- antes de que el nuevo plan fuera creado.
--
-- Problema: La función solo activaba el próximo plan como parte de la transición
-- de un plan activo. Si el plan anterior ya estaba completado y el nuevo se creó
-- después, no había nada que disparara la activación.
--
-- Fix: Agregar segunda rama que active planes próximos cuya fecha_inicio ya pasó
-- cuando no existe ningún plan activo.
...
BEGIN
  ...
  IF plan_activo_finalizado_id IS NOT NULL THEN
    ...
    END IF;
  ELSE
    -- FIX: Si no hay plan activo finalizado, verificar si existe algún plan activo
    SELECT EXISTS(
      SELECT 1 FROM public.planes_lectura WHERE estado = 'activo'
    ) INTO hay_plan_activo;

    -- Si NO hay ningún plan activo, buscar un plan próximo cuya fecha_inicio ya pasó
    IF NOT hay_plan_activo THEN
      SELECT id INTO plan_proximo_id
      FROM public.planes_lectura
      WHERE estado = 'proximo' AND fecha_inicio <= fecha_actual_venezuela
      LIMIT 1;
...
FIX: También activa planes próximos cuya fecha_inicio ya pasó cuando no hay ningún plan activo.
