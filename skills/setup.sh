#!/bin/bash
# Setup AI Skills for Kairos development
...
#   - Antigravity: .agent/skills/ symlink + .agent/rules/agent.md + GEMINI.md
...
show_help() {
    ...
    echo ""
    echo "Configure AI coding assistants for Kairos development."
    echo ""
    echo "Options:"
    echo "  --all         Configure all AI assistants"
    echo "  --claude      Configure Claude Code"
    echo "  --gemini      Configure Gemini CLI"
    echo "  --antigravity Configure Antigravity (Google)"
...
show_menu() {
    ...
    local options=("Claude Code" "Gemini CLI" "Antigravity (Google)" "Codex (OpenAI)" "GitHub Copilot")
...
setup_antigravity() {
    ...
    elif [ ! -d "$REPO_ROOT/.agent/rules" ]; then
        mkdir -p "$REPO_ROOT/.agent/rules"
    fi
    ...
    # Copy AGENTS.md to .agent/rules/agent.md
    if [ -f "$REPO_ROOT/AGENTS.md" ]; then
        cp "$REPO_ROOT/AGENTS.md" "$REPO_ROOT/.agent/rules/agent.md"
        echo -e "${GREEN}  ✓ AGENTS.md -> .agent/rules/agent.md${NC}"
    fi

    # Copy AGENTS.md to GEMINI.md (user rules)
    copy_agents_md "GEMINI.md"
}
...
if [ "$SETUP_CLAUDE" = false ] && [ "$SETUP_GEMINI" = false ] && [ "$SETUP_ANTIGRAVITY" = false ] && [ "$SETUP_CODEX" = false ] && [ "$SETUP_COPILOT" = false ]; then
...
[ "$SETUP_ANTIGRAVITY" = true ] && echo "  • Antigravity:    .agent/skills/ + .agent/rules/agent.md + GEMINI.md"
